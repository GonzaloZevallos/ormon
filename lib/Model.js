...
module.exports = class Model {
  ...
  validateInput (data) {
    ...
    const validationResult = validateTypes(this.structure, data);
    ...
    if (validationResult.hasUndeclaredFields) {
      for (const key in validationResult.undeclaredFields) {
        console.log(`'${key}' key was not defined on model structure`)
      }
      throw 'Wrong keys.';
    }

    return !validationResult.hasErrors && !validationResult.hasUndeclaredFields;
  }
  ...
  update(id, data) {
    ...
    const allDataModified = allData.map((product) => {
      ...
      this.validateInput(elementModified);
